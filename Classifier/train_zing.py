...
def dataio_prep(hparams):
   ...
   # Define label pipeline:
    @sb.utils.data_pipeline.takes("word")
    @sb.utils.data_pipeline.provides("word", "word_encoded")
    def label_pipeline(word):
        """Defines the pipeline to process the input speaker label."""
        yield word
        word_encoded = label_encoder.encode_label_torch(word)
        yield word_encoded
    ...
    for dataset in data_info:
        datasets[dataset] = sb.dataio.dataset.DynamicItemDataset.from_json(
            ...
            replacements={"data_root": hparams["data_folder"]},
            dynamic_items=[audio_pipeline, label_pipeline],
            output_keys=["id", "sig", "word_encoded"],
        )
